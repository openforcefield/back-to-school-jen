...
| `0b_openff_2.3-rc2` | `openff_unconstrained-2.3.0-rc2` | Stock OpenFF-2.3.0-rc2 | Bonds | 0.005 | 50 | mean | ✓ | mean | ✓ |
...
| `e_broad_spec_recursion1` | `e_broad_spec_recursion1/openff-2.3.0-bondtypes-recursion` | Broad spec with bond types, recursion | Bonds | 0.005 | 100 | mean | ✓ | mean | ✓ |

> `d_gen_bond_recursive` is incomplete; only the output `final-force-field.offxml` is preserved (no run scripts).

**Note:** `0b_openff_2.3-rc2`, `d2_gen_bond_recursive`, and `e_broad_spec_recursion1` use a bond `k` lower bound of **90** (not 0).
...
| e | `e_broad_spec_recursion1/openff-2.3.0-bondtypes-recursion.offxml` |
